...
namespace MusicPlayerTokenDealer
{
    class gblClass
    {
        ...
        {
            string validPasswordPattern = @"(?=^.{8,10}$)(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&*()_+}{:;'?/>.<,])(?!.*\s).*$";

            return new Regex(validPasswordPattern, RegexOptions.IgnoreCase);
        }

        internal static bool PasswordIsValid(string emailAddress)
        {
            bool isValid = ValidPasswordRegex.IsMatch(emailAddress);

            return isValid;
        }


        public DataTable fnFillDataTable_Online(string sSql)
        {
            SqlDataAdapter Adp = new SqlDataAdapter();
            DataTable mldData;
            try
            {
                Adp = new SqlDataAdapter(sSql, StaticClass.constrOnline);
                mldData = new DataTable();
                Adp.Fill(mldData);
            }
            catch (Exception ex)
        ...
        public DataSet fnFillDataSet_Online(string sQuery)
        {
            SqlDataAdapter Adp = new SqlDataAdapter();
            DataSet mlds;
            try
            {
                Adp = new SqlDataAdapter(sQuery, StaticClass.constrOnline);
                mlds = new DataSet();
                Adp.Fill(mlds);
            }
            catch (Exception ex)
