...
{
    public partial class frmTokenDistributionAsian : Form
    {
        ...
        public frmTokenDistributionAsian()
        {
            InitializeComponent();
        }
         private frmDealerAdministrator mainForm = null;
         public frmTokenDistributionAsian(Form callingForm)
        {
            mainForm = callingForm as frmDealerAdministrator;
            InitializeComponent();
        }  
        private void frmTokenDistributionCopyleft_Load(object sender, EventArgs e)
        {
            InitilizeTokenGenerationGrid();
            FillClient();
            dtpSanjivaniExpiryDate.Value = DateTime.Now.Date;
            ...
            sQrStr = sQrStr + " where year(DateTokenDefined)=" + DateTime.Now.Year + "  and isnull(Users.MusicType,'') <>'Copyright'  and AMPlayerTokens.ClientId=" + StaticClass.DfClient_Id + " ";
            sQrStr = sQrStr + " and AMPlayerTokens.IsSanjivani=1 order by Users.UserID desc  ";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            LocationName = txtLocation.Text;
                IsDam = 0;
            if (chkSanjivani.Checked == true)
            {
                IsSanjivani = 1;
                SanjivaniDateExpiry = dtpSanjivaniExpiryDate.Value;
            }
        ...
        private void SaveTokenGeneration(Int32 Return_UserId)
        {
            ...
            for (int i = 1; i <= TotalToken; i++)
            {
                ...
                cmd.Parameters["@IsSanjivani"].Value = "1";

                cmd.Parameters.Add(new SqlParameter("@SanjivaniExpiryDate", SqlDbType.DateTime));
                cmd.Parameters["@SanjivaniExpiryDate"].Value = dtpSanjivaniExpiryDate.Value;
        ...
        private void ClearFields()
        {
            ...
            sQr = sQr + " where year(DateTokenDefined)=" + DateTime.Now.Year + "  and isnull(Users.MusicType,'') <>'Copyright'  and AMPlayerTokens.clientid=" + StaticClass.DfClient_Id + " ";
        ...
        private void SendEmailUser()
        {
            try
            {
                ...
                if (IsDam == 1)
                {
                    body += " http://eufory.org/musicplayer/DAM/publish.htm \n";
                }
                if (IsSanjivani == 1)
                {
                    body += " http://eufory.org/musicplayer/AsianDirectLicense/publish.htm \n";
                }
                ...
                if (IsDam == 1)
                {
                    body += "Your Dam subscription expiry date: " + DamDateExpiry + " \n";
                }
        ...
        private void SendEmailMainClientData()
        {
            try
            {
                ...
                body += "3) Once the installation is started you will need to use Token and Clientname to complete the installation. \n";
                body += "Your installation user name: " + MainUserName + " \n";
                body += "Your dealer code: " + StaticClass.DealerCode + " \n";
        ...
        private void dgTokenGeneration_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 7)
            {
                ...
                sQr = sQr + " DateTokenExpire,NumberofTitles, PlayerType,Street, Cityname, TeamviewerId , TvPassword ,Users.Location,Users.vatnumber ";
        ...
        private void txtSearchUserName_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (txtSearchUserName.Text.Length >= 1)
                {
                    ...
                    sQr = sQr + " NumberofTitles, PlayerType from AMPlayerTokens ";
                    sQr = sQr + " inner join Users on AMPlayerTokens.UserId= Users.UserID ";
                    sQr = sQr + " where Users.UserName like '%" + txtSearchUserName.Text + "%'  and isnull(Users.MusicType,'') <>'Copyright' and AMPlayerTokens.clientId=" + StaticClass.DfClient_Id + "";
                    sQr = sQr + " and AMPlayerTokens.IsSanjivani=1  order by Users.UserID desc  ";
                    FillTokenGenerationData(sQr);
                }
                else
        ...
        private void cmbSearchPlayerType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cmbSearchPlayerType.Text != "")
            {
                ...
                sQr = sQr + " NumberofTitles, PlayerType from AMPlayerTokens ";
                sQr = sQr + " inner join Users on AMPlayerTokens.UserId= Users.UserID ";
                sQr = sQr + " where PlayerType='" + cmbSearchPlayerType.Text + "'  and isnull(Users.MusicType,'') <>'Copyright' and AMPlayerTokens.clientId=" + StaticClass.DfClient_Id + "";
                sQr = sQr + " and AMPlayerTokens.IsSanjivani=1  order by Users.UserID desc  ";
                FillTokenGenerationData(sQr);
            }
            else
